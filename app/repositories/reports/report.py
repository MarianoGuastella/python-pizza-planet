...
class MostRequestedIngredientStrategy(ReportStrategy):
    def generate(self):
        most_requested_ingredient = db.session.query(
            Ingredient.name, 
         ...
         .first()
        return {"name": most_requested_ingredient[0], "count": most_requested_ingredient[1]}
...
class TopCustomersStrategy(ReportStrategy):
    def generate(self):
        top_customers = db.session.query(
            Order.client_name, 
            func.count(Order._id).label('orders_count')
        ).group_by(Order.client_name)\
         .order_by(func.count(Order._id).desc())\
         .limit(3)\
         .all()
        return [{"client_name": customer[0], "orders_count": customer[1]} for customer in top_customers]
