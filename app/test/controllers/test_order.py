...
def test_create(app, ingredients, beverages, size, client_data):
    ...
    for param, value in order.items():
        ...
        ingredients_in_detail = set(item['ingredient']['_id'] for item in created_order['ingredient_detail'] if not item['ingredient'] == None)
        pytest.assume(not ingredients_in_detail.difference(ingredient_ids))
        beverages_in_detail = set(item['beverage']['_id'] for item in created_order['beverage_detail'] if not item['beverage'] == None)
...
def test_calculate_order_price(app, ingredients, beverages, size, client_data):
    ...
    pytest.assume(created_order['total_price'] == round(created_size['price'] + sum(ingredient['price'] for ingredient in created_ingredients) 
...
def test_create_empty_ingredients_list(app, size, client_data):
    created_size, _, _ = __create_sizes_ingredients_beverages([], [], [size])
